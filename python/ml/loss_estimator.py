...
def compute_yield_loss_and_economics(
    ...
    projected_normal_total_tons = baseline_yield * area
    ...
    if calamity_damage_percent is not None and calamity_damage_percent > 0:
        ...
        predicted_remaining_total_tons = max(0.0, projected_normal_total_tons * (1.0 - damage_pct / 100.0))
        predicted_yield_reduction_percent = damage_pct
        yield_reduction_tons = projected_normal_total_tons - predicted_remaining_total_tons
    else:
        # Compare predicted yield vs baseline
        predicted_total_tons = predicted_yield * area
        if predicted_total_tons < projected_normal_total_tons:
            yield_reduction_tons = projected_normal_total_tons - predicted_total_tons
            predicted_yield_reduction_percent = (yield_reduction_tons / projected_normal_total_tons) * 100.0
    ...
    if crop_unit_price_php_per_kg is not None and crop_unit_price_php_per_kg > 0:
        ...
        yield_reduction_kg = yield_reduction_tons * 1000.0
