...
export const FarmerForm: React.FC<FarmerFormProps> = ({
  ...
  const isFormEdit = isEdit || isEditing;
  ...
  const [formData, setFormData] = useState({
    ...
    farmerCode: initialData?.farmerCode || "Farmer",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isFormEdit
        ? `/api/rsbsa/farmers/${initialData.id}`
        : `/api/rsbsa/farmers`;
      ...
      if (!res.ok) {
        throw new Error(data.error || "Failed to save farmer record");
      }

      setServerSuccess(
        isFormEdit
          ? "Farmer profile updated successfully!"
          : "New farmer registered successfully in RSBSA database!"
      );
      ...
      if (onSuccess) {
        ...
        setTimeout(() => {
          router.push(
            isFormEdit
              ? `/staff/farmers/${initialData.id}`
              : `/staff/farmers/${data.id || ""}`
          );
          router.refresh();
        }, 1000);
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3.5 items-start">
        ...
        <div className="lg:col-span-2 lg:row-start-1 lg:col-start-1 rounded-xl border border-slate-200/80 bg-white p-3.5 md:p-4 shadow-2xs space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500">
                  DA-RSBSA registry classification.
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                {FARMER_SECTOR_CATEGORIES.map((cat) => (
        ...
        {/* DIV 2: PERSONAL PROFILE & DEMOGRAPHICS (grid-area: 2 / 1 / 3 / 3) */}
